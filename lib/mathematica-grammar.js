...
export default class MathematicaGrammar extends Grammar {
  ...
  editorChanged(editor,subscriptions) {
    ...
    if (this.parserCache[bufferID] && (this.parserCache[bufferID].parser instanceof MathematicaParser)) {
      ...
      if (!tokenBuffer.id) {
        ...
        if (editorBuffer.tokenizedBuffers) {
          editorBuffer.tokenizedBuffers.push(tokenBuffer);
        } else {
          editorBuffer.tokenizedBuffers = [tokenBuffer];
        }
      }
      return
    }
  ...
  bufferDidChangeText(parser) {
    ...
    return function (e) {
      let buffer = p.buffer;
      let tokenizedBuffers = buffer.tokenizedBuffers;
      let invalidatedLines = p.tokenizeChanges(e)
      buffer.unparsedLines = invalidatedLines;
      //console.log(["tokenInvalid:",invalidatedLines]);
      
      tokenizedBuffers.forEach(function(tb) {
        if (!(tb.isAlive())) {
          return;
        }
        tb.invalidRows = invalidatedLines;
        tb.invalidateRow(Math.min(...invalidatedLines));
      })
    }
  }
  ...
  bufferDidStopChanging(parser) {
    ...
    return function (e) {
      ...
      let unparsedLines = buffer.unparsedLines;
      
      let allLines = invalidatedLines;
      if (unparsedLines && unparsedLines.length > 0) {
        allLines = invalidatedLines.concat(unparsedLines);
      } else {
        allLines = invalidatedLines;
      }
      ...
      tokenizedBuffers.forEach(function(tb) {
        ...
        tb.invalidRows = uniqueLines;
        
        tb.invalidateRow(Math.min(...uniqueLines));
      });
      
      //console.log("done parsing");
      buffer.unparsedLines = [];
    }
  }
}