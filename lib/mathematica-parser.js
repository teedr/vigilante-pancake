...
export default class MathematicaParser {
  constructor (buffer) {
    ...
    this.buffer = buffer
    this.tokenizedBuffer = buffer.tokenizedBuffer
  ...
  updateChanges(e) {
    ...
    e.changes.map(function(change) {
      let invalid = this.updateTokens(change);
  ...
  updateChange(e) {
    ...
    let startPoint = e.oldRange.start
    let endPoint = e.oldRange.end
    
    let startIndex = this.buffer.characterIndexForPosition(startPoint)
    let endIndex = this.buffer.characterIndexForPosition(endPoint)
    
    let length = endIndex - startIndex;
    
    var changedRegion = this.parser.bufferChangedSync(bufferText, startIndex, length, "", false);
    
    var rootStartRow = this.buffer.positionForCharacterIndex(changedRegion[0]).row
    var rootEndRow = this.buffer.positionForCharacterIndex(changedRegion[0]+changedRegion[1]).row
    
    let endRow = this.buffer.positionForCharacterIndex(endIndex-1).row;
  ...
  updateTokens(change) {
    ...
    var oldToken = this.parser.getTokenForOffsetSync(endIndex);
    var oldTokenEndRow = (this.buffer.positionForCharacterIndex(oldToken.end).row)
        
    var changedRegion = this.parser.bufferChangedSync(bufferText, startIndex, length, change.newText, true)
    
    let rowStartIndex = this.buffer.characterIndexForPosition(this.buffer.rangeForRow(startPoint.row).start)
    let rowEndIndex = this.buffer.characterIndexForPosition(this.buffer.rangeForRow(endPoint.row).end)-1
    let startToken = this.parser.getTokenForOffsetSync(rowStartIndex)
    let endToken = this.parser.getTokenForOffsetSync(rowEndIndex)
    
    let invalidatedLines = []
    
    let invalidStartRow = this.buffer.positionForCharacterIndex(startToken.start).row
    let invalidEndRow = (this.buffer.positionForCharacterIndex(endToken.end).row)
    ...
    for (var i = rootStartRow; i < rootEndRow; i++) {
      ...
      for (var t = 0; t < invalidatedLines.length; t++) {
        if (invalidatedLines[t] == i) {
    ...
    invalidatedLines = invalidatedLines.concat(invalidatedScopeLines);
    if (this.debug) {
      console.log(["invalidated:",invalidatedLines]);
    }
    return invalidatedLines
  }
  ...
  // parseChanges() {
  //   let parsedLines = this.unparsedLines;
  //   this.parser.parseChangesSync();
  //   this.unparsedLines = [];
  //   console.log(["invalidated scope lines:",parsedLines]);
  //   return parsedLines;
  // }
  ...
  parseChange(change) {
    ...
    //console.log(["invalidated scope lines:",invalidatedLines]);
    if (changedRegion[0] == -1) {
      return invalidatedLines
    }
  ...
  tokensForLine(line) {
    ...
    // 
    // let lineStartTokenIndex = this.parser.getTokenIndexForOffsetSync(lineStartIndex)
    // let lineEndTokenIndex = this.parser.getTokenIndexForOffsetSync(lineEndIndex)
    // 
    // let uniqueTokenIndices = []
    // for (var i = lineStartTokenIndex; i <= lineEndTokenIndex; i++) {
    //   uniqueTokenIndices.push(i)
    // }
    // 
    // let tokensOnLine = []
    // for (var i = 0; i < uniqueTokenIndices.length; i++) {
    //   let tok = this.parser.getTokenForIndexSync(uniqueTokenIndices[i])
    //    if (!this.unrecognizableTokenTypes.includes(tok.scopeInt) && tok.text != null && tok.start < lineEndIndex) {
    //     tokensOnLine.push(tok)
    //   }
    // }
